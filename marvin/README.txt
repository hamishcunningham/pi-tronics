...
- /etc/default/marvind -- a set of config variables and support code (see
  ./marvind.default)
- /usr/sbin/marvin     -- the daemon itself (./marvin)
- /etc/init.d/marvind  -- an init interface used by the operating system to
  start/stop/etc. the daemon (./marvind.init)

There are also:

- links to the init interface from /etc/rc?.d which are managed by update-rc.d
- (when running) a process ID file in /var/run/marvin.pid
- TODO delete? (when running) a "first run after boot" indicator file in /tmp
...
contains monitor-log.sh which greps the most recent marvin entries from
